...
import { Skeleton, SkeletonDetailBlock } from '../ui/Skeleton';
...
interface ReferralDetailProps {
  referralId: string;
  /** 'page': full route view (back button, print header, max-width, bottom padding for the mobile tab bar).
   *  'pane': embedded in a desktop master-detail right pane -- no back button, no outer width cap. */
  variant?: 'page' | 'pane';
}

export const ReferralDetail: React.FC<ReferralDetailProps> = ({ referralId, variant = 'page' }) => {
  ...
  const outerClass = isPane
    ...
    : 'space-y-6 pb-16 sm:pb-0 max-w-5xl mx-auto print:max-w-none print:pb-0 print:m-0 print:space-y-4';
  ...
  if (!referral && loading) {
    return (
      <div className={isPane ? 'space-y-6' : 'space-y-6 max-w-5xl mx-auto'}>
        <div className="flex items-center gap-3">
          <Skeleton className="h-8 w-64" />
        </div>
        <div className={isPane ? 'space-y-6' : 'grid grid-cols-1 lg:grid-cols-3 gap-6'}>
          <div className={isPane ? 'space-y-6' : 'lg:col-span-2 space-y-6'}>
  ...
  return (
    <div className={outerClass}>
      ...
      <div className={isPane ? 'rounded-xl overflow-hidden print:hidden' : '-mt-4 sm:mt-0 -mx-4 sm:mx-0 sm:rounded-xl sm:overflow-hidden print:hidden'}>
